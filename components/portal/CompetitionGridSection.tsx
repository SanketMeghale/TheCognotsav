  ...
  return (
    <section id="registration-panel" className="rounded-[2rem] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.04),rgba(9,13,24,0.72))] p-4 sm:p-5 md:p-6">
      <div className="text-center">
        <p className="text-[11px] uppercase tracking-[0.22em] text-cyan-200/80">Events</p>
        <h3 className="mt-3 text-2xl font-semibold text-white sm:text-3xl">Choose your competition.</h3>
        ...
        {loadingEvents
          ? Array.from({ length: 6 }).map((_, index) => (
              <div key={index} className="rounded-[1.8rem] border border-white/10 bg-white/[0.04] p-4">
                <div className="h-48 animate-pulse rounded-[1.4rem] bg-white/10" />
          ...
          : events.map((event) => {
              ...
              return (
                <button
                  ...
                  className={`group overflow-hidden rounded-[1.9rem] border bg-[linear-gradient(180deg,rgba(255,255,255,0.06),rgba(9,13,24,0.9))] text-left transition duration-200 ${
                    active ? 'border-white/24' : 'border-white/10 hover:border-white/18'
                  } ${theme.glow}`}
                >
